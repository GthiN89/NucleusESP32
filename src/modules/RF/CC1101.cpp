...
void CC1101_CLASS::filterSignal() {
    ...
    uint32_t* minPtr = std::min_element(std::begin(diffs), std::end(diffs));
    ...
    if(checkReversed(rep2)){reverseLogicState();};

    if(DURATION_DIFF(pulses[0], rep1) < 40 and DURATION_DIFF(pulses[1], rep2) < 80) {
        filterAll();
    } else {
        pulses.clear();
