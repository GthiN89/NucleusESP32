...
void create_c1101_screen(void)
{
    ...
    // ta = lv_textarea_create(topLabel_container);  // Use global `ta` for frequency input
    // lv_textarea_set_password_mode(ta, true);
    // //lv_obj_add_event_cb(ta, ta_event_cb, LV_EVENT_ALL, kb);  // Event for showing the keyboard
    // lv_obj_set_size(ta, 80, 20);
    // lv_textarea_set_placeholder_text(ta, "433.92");
    // lv_textarea_set_max_length(ta, 6);
    // lv_textarea_set_one_line(ta, true);
    // lv_textarea_set_text(ta, frequency_buffer);  // Set the initial frequency from the buffer
    ...
    // text_area = lv_textarea_create(c1101AnaScreen);  // Store reference to text_area globally
    // lv_obj_set_size(text_area, 240, 180);  // Manually set width to 240px and height to 180px
    // lv_obj_align(text_area, LV_ALIGN_CENTER, 0, -20);  // Align the text area at the center with vertical offset
    // lv_textarea_set_text(text_area, "Welcome to the RF scanner.\nPlease set frequency.\n");
...
//         lv_textarea_add_text(text_area, "Initializing C1101\n");  // Use the global text_area reference
//         Serial.println("Initializing C1101");
//         lv_textarea_set_cursor_pos(text_area, LV_TEXTAREA_CURSOR_LAST);  // Ensure cursor is at the end
//         lv_obj_scroll_to_view_recursive(text_area, LV_ANIM_ON);  // Ensure the view scrolls to the last line
...
//                 if(initCC1101()){
//                     lv_textarea_add_text(text_area, "CC1101 Initialized\n");
//                 } else {
//                     lv_textarea_add_text(text_area, "CC1101 Connection Error");
//                 }
...
//        ProtAnaRxEn = true;
//        lv_textarea_add_text(text_area, "Status:ready\n");  // Use the global text_area reference
                
//         Serial.println("C1101 inicialized");

//     } else {
//         Serial.println("Error: text_area is NULL");
//     }
// }
// }
...
//         lv_obj_t* text_area; // = ScreenManager::getInstance().getTextArea();
//         if (text_area) {
//             char buffer[32]; 
//             Serial.println("Received Value:");
//             Serial.println(receivedValue);
//             lv_textarea_add_text(text_area, "Value: ");
...
//             lv_textarea_add_text(text_area, "Length: ");
...
//             lv_textarea_add_text(text_area, "Protocol: ");
...
// void ProtAnalyzerloop() {
//     ProtAnaRxEn = true;
//     if (mySwitch.available() && recievedSubGhz == false) {
//         receivedValue = 0; 
//         receivedBitLength = 0;       
//         receivedProtocol = 0;       
//         recievedSubGhz = true;
...
//     Serial.println(receivedBitLength);
//     lv_textarea_add_text(text_area, "Lenght: ");
//     sprintf(buffer, "%d", receivedBitLength); 
//     lv_textarea_add_text(text_area, buffer);  
//     lv_textarea_add_text(text_area, "\n");
//     receivedProtocol = mySwitch.getReceivedProtocol(); 
...
//     lv_textarea_add_text(text_area, buffer);  // Add the converted string to the text area
//     lv_textarea_add_text(text_area, "\n");
//     }
...
//     strncpy(filename_buffer, lv_textarea_get_text(ta_filename), sizeof(filename_buffer) - 1);
...
//         lv_textarea_add_text(text_area, "Saving file: ");
//         lv_textarea_add_text(text_area, filename_buffer);
//         lv_textarea_add_text(text_area, "/n");
...
//                 lv_textarea_add_text(text_area, "SD initialization failed!\n");
...
//                 lv_textarea_add_text(text_area, "Signal saved to SD card as ");
//                 lv_textarea_add_text(text_area, filename_buffer);
//                 lv_textarea_add_text(text_area, "\n");
...
//             } else {
//                 lv_textarea_add_text(text_area, "Failed to open file for writing.\n");
...
void fcnProtAnaPlay(lv_event_t * e) {
    ...
    //     	lv_event_code_t code = lv_event_get_code(e);
    // if (code == LV_EVENT_CLICKED) {
    //     lv_textarea_add_text(text_area, "Sending code\n");  // Use the global text_area reference
    //     lv_textarea_set_cursor_pos(text_area, LV_TEXTAREA_CURSOR_LAST);  // Ensure cursor is at the end
    //     lv_obj_scroll_to_view_recursive(text_area, LV_ANIM_ON);  // Ensure the view scrolls to the last line
    ...
    // lv_textarea_add_text(text_area, "Code send\n");  // Use the global text_area reference
    // }
