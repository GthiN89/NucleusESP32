...
#include <atomic>  // Include the standard atomic library
...
class IRrecv {
 ...
 private:
  ...
  bool matchAtLeast(const uint32_t measured, const uint32_t desired,
                    const uint8_t tolerance = kUseDefTol,
                    const uint16_t delta = 0);
  uint16_t _matchGeneric(uint16_t *data_ptr,
  ...
  uint16_t matchBytes(uint16_t *data_ptr, uint8_t *result_ptr,
                      ...
                      const bool MSBfirst = true,
                      const bool expectlastspace = true);
  uint16_t matchGeneric(uint16_t *data_ptr,
                        uint64_t *result_ptr,
  ...
  uint16_t matchGenericConstBitTime(uint16_t  *data_ptr,
                                    ...
                                    const int16_t excess = kMarkExcess,
                                    const bool MSBfirst = true);
  uint16_t matchManchesterData(uint16_t *data_ptr,
                               uint64_t *result_ptr,
                               const uint16_t remaining,
                               const uint16_t nbits,
  ...
  uint16_t matchManchester(uint16_t *data_ptr,
                           uint64_t *result_ptr,
                           const uint16_t remaining,
                           const uint16_t nbits,
                           const uint16_t hdrmark,
